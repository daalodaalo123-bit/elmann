    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            for (const it of input.items) {
                ...
                if (prod.archived)
                    throw new Error(`Product is archived: ${it.product_id}`);
...
export async function getSalesHistory(queryArg) {
...
export async function getSaleByReceipt(receipt_ref) {
...
export async function deleteSaleByReceipt(receipt_ref) {
    const session = await mongoose.startSession();
    try {
        let result;
        await session.withTransaction(async () => {
