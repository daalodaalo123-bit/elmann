...
import { requireAuth, requireRole, type RequestWithUser } from './middleware/authz.js';
import {
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'customer.create', 'customer', String(id));
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'customer.update', 'customer', String(req.params.id), parsed.data);
    res.json({ ok: true });
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await audit(req, (req as any).user ?? null, 'expense.create', 'expense', String(id), parsed.data);
      res.status(201).json({ id });
    } catch (e: any) {
...
app.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.json(rows.map((u: any) => ({ id: String(u._id), username: u.username, role: u.role, created_at: u.created_at })));
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await audit(req, (req as any).user ?? null, 'user.create', 'user', String(u._id), { username, role: parsed.data.role });
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'user.update', 'user', String(req.params.id), { ...parsed.data, password: parsed.data.password ? '***' : undefined });
    res.json({ ok: true });
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'product.create', 'product', String(id), parsed.data);
    res.status(201).json({ id });
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'product.restock', 'product', String(req.params.id), parsed.data);
    res.json({ ok: true });
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'product.decrease', 'product', String(req.params.id), parsed.data);
    res.json({ ok: true });
  })
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await audit(req, (req as any).user ?? null, 'sale.create', 'sale', String(result?.sale_id ?? ''), { receipt_ref: result?.receipt_ref });
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await audit(req, (req as any).user ?? null, 'sale.refund', 'sale', receipt_ref, parsed.data);
    res.json(result);
  })
);
...
app.use((err: any, _req: Request, res: Response, _next: NextFunction) => {
  ...
  if (isDbConn) {
    return res.status(503).json({
      error:
        'Database connection failed. Check server env MONGODB_URI (Atlas username/password, IP allowlist, and that you pasted only the URI value).'
    });
  }
